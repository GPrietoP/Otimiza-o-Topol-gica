...
## Projeto de Iniciação Ciêntífica 2021
...
def main(nelx,nely,nelz,volfrac,penal,rmin,ft):	
	...
	il, jl, kl = np.meshgrid(nelx, 0, np.linspace(0,nelz,num=nelz+1))           # Coordenadas
	...
	F = np.zeros((ndof,1))
	for x in range(len(loaddof[0,0])):      # Definindo a intensidade da força
		n = int(loaddof[0,0,x])		
		F[n,0] = -1                         
	U = np.zeros((ndof,1))	
	...
	while change>tolx and loop<maxloop:
		...
		print(f"Tempo: {fim2-ini2}")		
	fim1 = time.time()
	print(f"Tempo total: {fim1-ini1}")
		...
		for j in range(nelx):
			for k in range(nely):
				if xPhys[cont]>0.5:
					...
					total = total | cube										
				cont = cont+1
	ax.voxels(total, facecolors=colors, edgecolor='k')
	plt.axis('off')
	plt.show()
...
def oc(nelx,nely,nelz,x,volfrac,dc,dv,g):
	...
	def Mpre(M):
		'Construir Pré-condicionamento aqui'
	return ()

# O programa principal, caso não chamado no terminal    
