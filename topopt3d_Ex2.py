...
def main(nelx,nely,nelz,volfrac,penal,rmin,ft):	
	...
	il = nelx/2; jl = 0; kl = nelz/2          # Coordenadas
	...
	iif = np.array([0, 0, nelx, nelx]); jf = np.array([0, 0, 0, 0]); kf = np.array([0, nelz, 0, nelz])       # Coordenadas
	...
	while change>tolx and loop<maxloop:
		...
		# Solução do sistema
		ini2 = time.time()										
		U[freedofs,0], info = ssl.cg(K,F[freedofs,0], x0=None, tol=1e-8, maxiter=8000, M=None)		
		fim2 = time.time()
